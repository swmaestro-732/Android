...
import com.chillsam.courmy.course.domain.LoadDraftsUseCase
...
class DraftListViewModel
    ...
    constructor(
        ...
        init {
            // 콜드 스타트 직후에는 메모리가 비어 있다. 기기에 남은 초안을 올린다.
            viewModelScope.launch { loadDraftsUseCase() }
        }

        /** [draftId] 초안을 다음 코스 만들기 진입에서 이어서 편집하도록 지정한다. */
        fun beginEdit(draftId: String) = beginEditDraftUseCase(draftId)

        fun delete(draftId: String) = deleteDraftUseCase(draftId)
    }
